...
class InterventiApertiPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    var filtroApplicato = ref.read(filtroApplicatoProvider.notifier).state;
            ...
            children: [
              Row(
                children: [
                  ElevatedButton.icon(
                    onPressed: () async {
                      onUpdatedPressed(context, ref, filtroApplicato);
                    },
                    style: buttonStyle,
                  ...
                  ElevatedButton.icon(
                    onPressed: () async {
                      // Se almeno uno dei campi ha un valore non vuoto
                      if (filtroApplicato == true) {
                        ...
                        ref.read(filtroApplicatoProvider.notifier).state =
                            false;
                      }
                    },
                    style: buttonStyle,
                    icon: const Icon(Icons.cancel),
                    label: const Text("Pulisci Filtro"),
                  ),
                ],
              ),
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        if (fromDateController.text.isNotEmpty ||
                            ...
                            nDocController.text.isNotEmpty ||
                            clienteController.text.isNotEmpty ||
                            rifMatricolaClienteController.text.isNotEmpty ||
                            telaioController.text.isNotEmpty) {
  ...
  Future<void> applyFilter({
    ...
    await ref.read(interventiStateRepositoryProvider.notifier).filterInterventi(
...
Future<void> _deleteIntervento(Intervento intervento, WidgetRef ref) async {
  await ref.read(interventoApertoStateProvider.notifier).removeIntervento(intervento);
}

void onUpdatedPressed(
    BuildContext context, WidgetRef ref, filtroApplicato) async {
  showDialog(
    context: context,
    ...
    builder: (BuildContext context) {
      ...
      void resetFilter() async {
        try {
          ...
          ref.read(filtroApplicatoProvider.notifier).state = false;
