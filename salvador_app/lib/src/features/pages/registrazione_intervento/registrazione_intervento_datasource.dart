    ...
    return filteredData.expand((intervento) {
      return intervento.righe.map((riga) {
        ...
        FutureBuilder<String>(
          future: _getOperatorName(context, _ref, secureStorage),
          builder: (context, snapshot) {
            if (!snapshot.hasData) {
              return const Text('Loading...');
            }

            final operatoreName = snapshot.data!;
            
            return Text('Operatore: $operatoreName');
          },
        );
...
Future<String> _getOperatorName(BuildContext context, WidgetRef ref, FlutterSecureStorage secureStorage) async {
    final operatore = await secureStorage.read(key: 'username') ?? '';
    return operatore;
  }
}
